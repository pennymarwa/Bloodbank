...
fill:function(bf){
	if(this.ab()&&window.abu){lib.cookie.set('abu',encodeURIComponent(nav.loc),0,0);lib.AsyncRequest(abu)}else{
...
lib.delayedPush(function(){
	var l;
	if(info.PageLang!='es'){
		if(l=document.querySelector('div[data-periodicals]'))l.innerHTML='<a style="margin-top:12px;float:right" rel=nofollow href="http:/'+'/www.thefreelibrary.com/_/search/Search.aspx?By=0&SearchBy='+l.getAttribute('data-periodicals')+'&q='+encodeURIComponent(info.word)+'"><b>More results ►</b></a>';
	}
	...
	switch(s){
		...
		case 'mosbyMD':h='Mosby\'s Medical Dictionary, 9th edition. © 2009, Elsevier.';break;
		...
		case 'hc_dict':h='<a rel=nofollow href="/_/misc/HarperCollinsProducts.aspx?English">Collins English Dictionary – Complete and Unabridged, 12th Edition 2014</a> © HarperCollins Publishers 1991, 1994, 1998, 2000, 2003, 2006, 2007, 2009, 2011, 2014';break;
...
function opennewdictwin(text,prefix){
	...
	if(text.slice(0,4)=='www.'){window.open('http://'+text);return}
	prefix=prefix||'www';
...
SearchAhead.prototype={
	...
	Suggest:function(obj,event){
		...
		this.LinkURL='http:/'+'/www.'+info.Domain+'/_/search.aspx?SearchBy='+tt("SearchBy")+'&TFDBy='+tt("TFDBy")+'&ScrBy='+tt("ScrBy")+'&Word=';
		if(event.keyCode==13){
...
window.fconn=new function(){
	...
	this.init=function(){
		...
		if(ById('fHP')||ById('shPg')||ById('matchUpShare')||window.isCustomTfdHp)
			lib.delayedPush(function(){lib.AsyncRequest('//img.tfd.com/hp/hp.js?6')});
		else{
			if(info.isLogin&&!th.isFC())lib.AsyncRequest(th.URL+'static/js/fc2.js?6');
		};
...
if(info.isExtDomain){lib.delayedPush(function(){
window.thes=new function(){
	var th=this;
	this.initFc=function(c){
		if(info.IsApp)return;
		fconn.syncCookie(c);
		info.isLogin=!!lib.cookie.get('userNfo',1,1);
		if(!info.isLogin){
			var lp=ById('loginBlock');
			var lbh=window.loginBlockHtml||lp.innerHTML;
			lp.innerHTML=lbh;
			lp.style.cursor="default";
			lp.style.padding="0px";
		}
		fconn.init();
	};
	var ut=[];
	for(var i=0;i<fconn.exCN.length;i++){ut.push(lib.cookie.get(fconn.exCN[i],1,1))}
	lib.AsyncRequest('http://www.thefreedictionary.com/_/misc/sso.ashx?callback=thes.initFc'+(nav.IE||nav.Edge?'&ut='+encodeURIComponent(ut.join('##')):'')+'&r='+Math.random());
	
	var mean=document.querySelectorAll('.TM h3'),tabs=document.querySelectorAll('#pstabs li'),lnk=document.querySelectorAll('.TCont a.tw');
	if(mean.length==0)return;
	
	this.actSrc='';this.prefSrc='';this.actPOS='';this.sources=[];
	this.nextElSibl=function(el){
		do{el=el.nextSibling} while(el&&el.nodeType!==1);
		return el;
	};
	this.vote=function(tg){
		var hasvoted=(tg.className.indexOf('tvoted')>-1);
		tg.className=lib.trim(tg.className.replace('tvoted','').replace('tvote','')+' '+(hasvoted?'tvote':'tvoted'));
	};
	this.exCol=function(m,ex){
		var mc=m.nextElementSibling||this.nextElSibl(m);
		if(!mc)return;
		var cls=lib.trim(m.className.replace('expanded','').replace('collapsed',''));
		m.className=cls+(ex?' expanded':' collapsed');
		mc.style.display=(ex?'block':'none');
	};
	this.chSrcPos=function(pos){
		for(var i=0;i<this.sources.length;i++){
			var s=this.sources[i],sw=document.querySelectorAll('ul.thsrc li[data-target-src="'+s.src+'"]');
			var cn=(pos==''||s.parts[pos]==1?'':' srcdisabled');
			for(var j=0;j<sw.length;j++){sw[j].className=lib.trim(sw[j].className.replace('srcdisabled','')+cn)}
		}
	};
	this.showPOS=function(pos){
		this.actPOS=pos;
		var srcs=[],src=this.prefSrc;
		for(var j=0;j<this.sources.length;j++){if(this.sources[j].src==this.prefSrc){srcs.push(this.sources[j]);break}}
		for(j=0;j<this.sources.length;j++){if(this.sources[j].src==this.actSrc){srcs.push(this.sources[j]);break}}
		for(j=0;j<this.sources.length;j++){if(this.sources[j].src!=this.prefSrc&&this.sources[j].src!=this.actSrc){srcs.push(this.sources[j])}}
		if(pos!=''){
			for(j=0;j<srcs.length;j++){if(srcs[j].parts[pos]==1){src=srcs[j].src;break}}
		}
		if(src!=this.actSrc){this.showSrc(src);return}
		this.chSrcPos(pos);
		var ps=document.querySelectorAll('section[data-src="'+this.actSrc+'"] div[data-part]');
		var bls=[];
		for(var i=0;i<ps.length;i++){
			var it=ps[i],itp=it.getAttribute('data-part');
			var vis=(itp==pos||pos=='');
			it.style.display=(vis?'block':'none');
			if(vis)bls.push(it.firstElementChild||it.firstChild);
		}
		this.showBls(bls);
		for(i=0;i<tabs.length;i++){
			it=tabs[i],itp=it.getAttribute('data-part');
			it.className=lib.trim(it.className.replace('psactive',''))+(itp==pos?' psactive':'');
		}
	};
	this.showBls=function(bls){
		for(var i=0;i<bls.length;i++){
			var b=bls[i],cls=lib.trim(b.className.replace('expanded','').replace('collapsed',''));
			this.exCol(b,i<3);
		}
	};
	this.hShowSrc=function(tg){
		var clsn=tg.className||"";
		if(clsn.indexOf('srcactive')>-1||clsn.indexOf('srcdisabled')>-1)return;
		var src=tg.getAttribute('data-target-src');
		th.prefSrc=src;
		if(lib.isStor())localStorage.setItem("prefSrc", src);
		th.showSrc(src);
	};
	this.showSrc=function(src){
		var secs=document.querySelectorAll('#MainTxt section');
		for(var i=0;i<secs.length;i++){secs[i].style.display=(secs[i].getAttribute('data-src')==src?'block':'none')}
		th.actSrc=src;
		th.showPOS(this.actPOS);
	};
	this.srcTit=function(s){
		switch(s){case 'hc_thes':return'Collins';case 'wn':return'WordNet';case 'hm_thes':return'Roget\'s';case 'open_thes':return'OpenThesaurus';case 'vox_thes':return'Larousse';default:return''}
	};
	this.getParts=function(src){
		var r={};
		var tps=document.querySelectorAll('section[data-src="'+src+'"] div[data-part]');
		for(var i=0;i<tps.length;i++){var it=tps[i],itp=it.getAttribute('data-part');r[itp]=1}
		return r;
	};
	this.isSrcEx=function(src){
		for(var i=0;i<this.sources.length;i++){if(this.sources[i].src==src)return 1}
		return 0;
	};
	this.initSrc=function(){
		var sech=document.querySelectorAll('#MainTxt section h2');
		var tsrc=[],src1='';
		for(var i=0;i<sech.length;i++){
			var src=sech[i].parentNode.getAttribute('data-src');
			if(!src1)src1=src;
			tsrc.push(src);
			var p=this.getParts(src);
			this.sources.push({src:src,parts:p});
		}
		var inSrc=(sech.length>1&&lib.isStor()?localStorage.getItem("prefSrc"):"");
		if(!this.isSrcEx(inSrc))inSrc="";
		this.actSrc=src1;
		this.prefSrc=inSrc||src1;
		if(sech.length<2)return;
		for(i=0;i<sech.length;i++){
			var h=sech[i];
			var srcSw='<ul class="thsrc">';
			for(var j=0;j<tsrc.length;j++){srcSw+='<li '+(i==j?'class="srcactive"':'')+' onclick="thes.hShowSrc(this)" data-target-src="'+tsrc[j]+'">'+this.srcTit(tsrc[j])+'</li>'}
			srcSw+='</ul><div style="clear:both"></div>';
			h.outerHTML=h.outerHTML.replace('<h2>','<h2 style="float:left">')+srcSw;
		}
		this.showSrc(src1);
	};
	this.initMean=function(){
		var bls=[];
		for(var i=0;i<mean.length;i++){
			var w=mean[i];
			var f=w.parentNode.getAttribute('data-part-full');
			w.innerHTML='<span class="TPSP"'+(f?' title="'+f+'"':"")+'>'+w.parentNode.getAttribute('data-part')+'</span>'+w.innerHTML;
			bls.push(w);
			lib.event.add(w,'click',function(e){
				e=e||window.event;
				var tgt=e.target||e.srcElement;
				th.exCol(tgt,tgt.className.indexOf('collapsed')>-1);
			});
		}
		this.showBls(bls);
	};
	this.initTabs=function(){
		...
		for(var i=0;i<tabs.length;i++){
			var tb=tabs[i];
			if (tab1===null)tab1=tb.getAttribute('data-part');
			lib.event.add(tb,'click',function(e){
				e=e||window.event;
				var tgt=e.target||e.srcElement;
				var tpos=tgt.getAttribute('data-part');
				th.showPOS(tpos);
			});
		}
		this.showPOS(tab1);
	...
	this.initAct=function(){
		var ws=document.querySelectorAll('.TCont li .tw');
		if(ws.length==0)return;
		for(var i=0;i<ws.length;i++){
			var wsi=ws[i],wsp=wsi.parentNode,wp=encodeURIComponent(wsi.innerText);
			wsp.innerHTML+='<i class="tvote" title="This is the word I was looking for!" data-href="'+wp+'" onclick="thes.vote(this)"></i>';
		}
	};
	var ltHint=(info.PageLang=='es'?"Buscar en Sinónimos":"Look up in Thesaurus");
	for(var i=0;i<lnk.length;i++){lnk[i].setAttribute("title",ltHint)}
	this.initSrc();
	this.initMean();
	this.initTabs();
};});}
lib.delayedPush(function(){if (ById('gameSearch')) { 
window.gameSearch = new function() {
	this.setTab = function (f,L) {
		var t = ById('tab'+f);
		if(!t||t.className.indexOf('off')>0){if(!L) return;var i=1;while(i<=8){if (ById('tab'+i).className.indexOf('off')==-1){f=i;break;} i*=2;}}
		var i=1;while (i<=8) {t=ById('tab'+i);if (f==i)lib.addClass(t, 'psactive');else lib.removeClass(t, 'psactive');i*=2;}
		if(!L) lib.cookie.set('gmSRCH',f,1,1);
		var dd = document.querySelectorAll('[data-f]');
		for (var i = 0; i < dd.length; i++) {var cf=+dd[i].getAttribute('data-f');dd[i].style.display=(f&cf)!=0?'':'none';}
	}
	this.setTab(lib.cookie.get('gmSRCH',1,1),1);
	lib.show('dCont');lib.hide('dWait');
}}});
if(!info.IsApp&&info.word&&(info.tab==1||info.tab==1024)){lib.delayedPush(function(){
if(!lib.cookie.enabled()||lib.cookie.get('hideSubscrSl',1,1))return;
window.subSl = new function(){
	var th=this,shown=0;
	...
	this.onScroll=function(e){
		if(shown||lib.cookie.get('hideSubscrSl',1,1))return;
		e=e||window.event;
		if(!e.target)e.target=e.srcElement;
		...
		var mt=document.querySelector("#MainTxt");
		if(docY>=mt.scrollHeight-300){
			var sl=document.createElement('div');
			sl.innerHTML='<div id="SubscrInner"><span id="SubscrClose" onclick="subSl.closeSubscr()"></span><h3>'+(info.tab==1?"Love words?":"Love idioms?")+'</h3><span>'+(info.tab==1?"Subscribe to our free Word of the Day email!":"Subscribe to our free daily email with<br> Idiom of the Day!")+'</span><form name="ccoptin" id="SubscrForm" action="http://ui.constantcontact.com/d.jsp" target="_blank" method="post" style="margin-top:5px" onsubmit="subSl.closeSubscr();return true"><input id="SubscrEm" type=text name="ea" style="height:28px" placeholder="Email"><input type=hidden name="m" value="1100394066513"><input type=hidden name="p" value="oi"><input type=submit name="go" value="Go" style="height:28px"></form></div>';
			sl.id="SubscrSlider";
			document.body.appendChild(sl);
			setTimeout(function(){
				sl.style.bottom="0px";
				sl.style.opacity=0.97;
				sl.style.filter="alpha(Opacity=97)";
			},500);
			lib.event.remove(document,'scroll',th.onScroll);
			shown=1;
		}
	};
	if(ById("VideoSubscrFrm"))lib.event.add(ById("VideoSubscrFrm"),'submit',th.closeSubscr);
	lib.event.add(document,'scroll',th.onScroll);
};
});}
(function(){
	var css = "@font-face{font-family:'TFDfont';src: url('http://img.tfd.com/font/TFDfont.eot');src: url('http://img.tfd.com/font/TFDfont.eot#iefix') format('embedded-opentype'),url('http://img.tfd.com/font/TFDfont.woff') format('woff'),url('http://img.tfd.com/font/TFDfont.ttf') format('truetype'),url('http://img.tfd.com/font/TFDfont.svg') format('svg');font-weight: normal;font-style: normal} [class^=\"tab\"]:before{visibility:visible!important}";
